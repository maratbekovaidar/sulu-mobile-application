...
class UserProvider {
  ...
  Future<bool> login(String username, String password) async {
    ...
    var response = await http.post(url,
        body: jsonEncode({"password": password, "phoneNumber": username}),
  ...
  Future<UserModel> getLoggedUser() async {
    ...
    var url =
        Uri.parse('${Configuration.host}private/client/getLoggedUserInfo');
  ...
  /// Register
  Future<int> register(String firstName, String lastName, String patronymic,
      String phoneNumber, String password, int cityId) async {
  ...
  /// Send OTP code and Phone Number to backend
  Future<int> sendOtp(String phoneNumber) async {
    try {
      ...
      var url = Uri.parse(
          '${Configuration.host}public/auth/addUserToVerification?code=$code&phoneNumber=$phoneNumber');
  ...
  /// Check for existing the phone number
  Future<int> confirmPhoneNumber(String phoneNumber) async {

    var url = Uri.parse(
        '${Configuration.host}public/auth/checkPhoneNumber?phoneNumber=%2B$phoneNumber');

    print(url);
    var response =
        await http.post(url, headers: {'Content-Type': 'application/json'});

    print("Confirm phone number " + response.body);

    return jsonDecode(response.body)["httpStatus"];
  }

  /// Confirm Otp from backend
  Future<int> confirmOtp(String phoneNumber, String userCode) async {
    var url = Uri.parse(
        '${Configuration.host}public/auth/verifyPhoneNumber?code=$userCode&phoneNumber=$phoneNumber');
  ...
  /// Get OTP code for sending to backend
  Future<String> getOtp(String phoneNumber) async {
    var url = Uri.parse(
        'https://smsc.kz/sys/send.php?login=info@sulu.life&psw=c708adb7a37585ca85de3ba573feb71aa1e57cf2&phones=$phoneNumber&mes=code&call=1');
    var response = await http.post(url);
  ...
  Future<bool> changeUserInfo(
      String firstName, String lastName, String photo) async {
    String token;
    token = (await storage.read(key: 'token'))!;
  ...
  Future<bool> changeUserPassword(
    ...
    token = (await storage.read(key: 'token'))!;
    var url = Uri.parse(
        '${Configuration.host}private/user/changePassword?newPassword=$newPassword&oldPassword=$oldPassword');
  ...
  Future<List<CityModel>> getCities() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
          "Bad Request http status " + response.statusCode.toString());
    }
  }
  ...
  Future<bool> changeCity(int cityId) async {
    ...
    token = (await storage.read(key: 'token'))!;
    var url = Uri.parse(
        '${Configuration.host}private/user/changeUserCityByUserToken?id=$cityId');
